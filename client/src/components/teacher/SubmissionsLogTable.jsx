          ...
          <tbody className="divide-y divide-slate-200 font-medium">
            {submissionsList.length > 0 ? (
              submissionsList.map(sub => (
                <tr key={sub.id} className={sub.status === 'Correction Required' || sub.status === 'CORRECTION_REQUIRED' ? 'bg-red-50/60' : 'hover:bg-slate-50'}>
                  ...
                  <td className="p-3 font-semibold text-slate-700">{sub.course || sub.class || '-'}</td>
                  <td className="p-3 font-semibold text-slate-600">{sub.examType || 'Regular'}</td>
                  <td className="p-3 text-center font-bold">{sub.totalStudents || sub.students || 0}</td>
                  <td className="p-3 text-center">
                    <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold ${sub.status === 'Approved' || sub.status === 'Published' || sub.status === 'APPROVED' || sub.status === 'PUBLISHED' ? 'bg-emerald-100 text-emerald-800' :
                      sub.status === 'Correction Required' || sub.status === 'CORRECTION REQUIRED' || sub.status === 'CORRECTION_REQUIRED' ? 'bg-red-100 text-red-800 border border-red-200' : 'bg-amber-100 text-amber-800'
                      }`}>
                      {sub.status}
                    </span>
                  </td>
                  <td className="p-3 text-center">
                    {sub.status === 'Correction Required' || sub.status === 'CORRECTION REQUIRED' || sub.status === 'CORRECTION_REQUIRED' ? (
                      <button
